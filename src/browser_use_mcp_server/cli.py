...
def import_server_module():
    ...
    try:
        ...
        try:
            ...
            if os.path.exists(os.path.join(root_dir, 'server', 'server.py')):
                spec = importlib.util.spec_from_file_location(
                    "server.server", 
                    os.path.join(root_dir, 'server', 'server.py')
                )
        ...
        raise ImportError("Could not find server module. Make sure it's installed correctly.")
...
def run(subcommand, port, chrome_path, window_width, window_height, locale, task_expiry_minutes, stdio):
    ...
    if subcommand != 'server':
        click.echo(f"Unknown subcommand: {subcommand}. Only 'server' is supported.", err=True)
        sys.exit(1)
